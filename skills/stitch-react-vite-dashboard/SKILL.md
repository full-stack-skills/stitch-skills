...
当用户需要 **为当前请求选择并执行可验证、可恢复的专业工作流** 时加载本技能。先从请求中提取目标、输入、约束、交付格式和验收标准；描述摘要为：把 Stitch 设计实现为 React/Vite 数据看板；当需 TanStack Query、表格、筛选、异步状态和设计 token 一致性时触发。只有普通展示组件用 stitch-react-components；链上读数仅在明确请求时加入。。
